...
for (( ;; )) ; do
	...
	# Uptime
	up 56 "<p>`uptime -p`</p>" $www > /tmp/M.SER/index1.html

	# CPU Usage
	cpu=`iostat | grep -A1 '%idle' | tr " " : | tr -s : | cut -d : -f 7 | tail -n1 | cut -d . -f 1`
	cpu=`expr 100 - $cpu`
	up 65 "<p>$cpu Percent</p>" /tmp/M.SER/index1.html > /tmp/M.SER/index2.html
	rm -f /tmp/M.SER/index1.html

	# RAM Usage
	ram=`free -m | grep Mem | tr -s " " : | cut -d : -f 3`
	up 74 "<p>$ram Megabyte</p>" /tmp/M.SER/index2.html > /tmp/M.SER/index3.html
	...
	up 92 "<p>$n Person</p>" /tmp/M.SER/index4.html > /tmp/M.SER/index5.html
	rm -f /tmp/M.SER/index4.html

	# SSH Attacks
	n=`grep 'Failed password' /var/log/auth.log | wc -l`
	up 101 "<p>$n SSH Attack on server</p>" /tmp/M.SER/index5.html > /tmp/M.SER/index6.html
	rm -f /tmp/M.SER/index5.html

	# Disk Usage
	n=`df -T | grep "$root" | tr -s " " | rev | cut -d ' ' -f 2 | rev | tr -d '%'`
	up 110 "<p>$n Percent</p>" /tmp/M.SER/index6.html > /tmp/M.SER/index7.html
	rm -f /tmp/M.SER/index6.html

	# Ping Latency
	Latency=`ping -c 1 8.8.8.8 | grep 'time=' | cut -d '=' -f 4`
	up 119 "<p>$Latency from 8.8.8.8</p>" /tmp/M.SER/index7.html > /tmp/M.SER/index8.html
	rm -f /tmp/M.SER/index7.html

	# Last Update
	time=`date +'%e %b %Y - %H:%M:%S'`
	up 130 "<p>Last Update : $time</p>" /tmp/M.SER/index8.html > /tmp/M.SER/index9.html
